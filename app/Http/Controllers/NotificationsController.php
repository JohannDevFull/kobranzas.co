...
class NotificationsController extends Controller
{
    ...
    public function getAll(Request $request)
    {
        ...
        if (Auth::user()->roles[0]->name == 'Admin') {
            $not = Notification::orderBy('id','DESC')->paginate($show);
            ...
            $not = Notification::where('info', 'like', '%\\"building_id\\":' . $admin_building->id_building . ',%')->orderBy('id','DESC')->paginate($show);
        }
        return [
            'pagination' => [
                'total' => $not->total(),
                'current_page' => $not->currentPage(),
                'per_page' => $not->perPage(),
                'last_page' => $not->lastPage(),
                'from' => $not->firstItem(),
                'to' => $not->lastPage(),
